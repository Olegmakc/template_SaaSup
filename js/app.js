(() => {
    ...
    function initSliders() {
        if (document.querySelector(".testimonials__slider")) new core(".testimonials__slider", {
            ...
            navigation: {
                prevEl: ".testimonials__controls .slider-controls__btn_prev",
                nextEl: ".testimonials__controls .slider-controls__btn_next"
            },
            on: {}
        });
    }
    ...
    if (document.querySelectorAll("[data-prlx-parent]")) {
        const mQuery = window.matchMedia("(min-width: 767px)");
        mQuery.matches ? modules_flsModules.parallax = new parallax_Parallax(document.querySelectorAll("[data-prlx-parent]")) : null;
        mQuery.addEventListener("change", (function(e) {
            if (e.matches) modules_flsModules.parallax = new parallax_Parallax(document.querySelectorAll("[data-prlx-parent]")); else {
                modules_flsModules.parallax.elements[0].destroyEvents();
                let prlItems = modules_flsModules.parallax.elements[0].elements;
                for (let index = 0; index < prlItems.length; index++) {
                    const prlItem = prlItems[index];
                    prlItem.style.transform = "";
                }
            }
        }));
    }
    ...
    function documentLoaded(e) {
        ...
        const hederStartPoint = header.dataset.scroll ? header.dataset.scroll : 1;
        addClassToHederIfSectionScrolled(header, hederStartPoint);
        document.addEventListener("fpswitching", (e => {
            let activeSectionId = e.detail.fp.activeSectionId;
            if (activeSectionId >= 1) {
                if (!header.classList.contains("_dark-theme")) {
                    header.classList.add("_dark-theme");
                    header.classList.add("_header-scroll");
                }
            } else addClassToHederIfSectionScrolled(header, hederStartPoint);
        }));
        const pageUrl = window.location.pathname;
        if (-1 == pageUrl.indexOf("home")) header.classList.add("_dark-theme");
    }
    function documentClicked(e) {
        let target = e.target;
        if (target.closest("button.menu__link") && window.innerWidth > 767.98) {
            const activeLink = document.querySelector(".menu__item._show-submenu");
            activeLink && activeLink.querySelector(".menu__link") != target ? activeLink.classList.remove("_show-submenu") : null;
            target.closest(".menu__item").classList.toggle("_show-submenu");
        } else if (!target.closest(".menu__item") && document.querySelector(".menu__item._show-submenu")) document.querySelector(".menu__item._show-submenu").classList.remove("_show-submenu");
    }
    function addClassToHederIfSectionScrolled(header, hederStartPoint) {
        let activeSection = modules_flsModules.fullpage.activeSection;
        if (activeSection) {
            if (hederStartPoint > activeSection.scrollTop) {
                header.classList.remove("_header-scroll");
                header.classList.remove("_dark-theme");
            }
            activeSection.addEventListener("scroll", (e => {
                if (hederStartPoint < activeSection.scrollTop) {
                    header.classList.add("_header-scroll");
                    header.classList.add("_dark-theme");
                } else {
                    const pageUrl = window.location.pathname;
                    if (pageUrl.indexOf("home") >= 0) header.classList.remove("_dark-theme");
                    header.classList.remove("_header-scroll");
                }
            }));
        }
